...
The milestones below record earlier deliveries. Video and occurrence reviews now use the filter-driven workspace described in the README: complete URL queries, cursor-only Skip, local tag editing, refreshed queue membership after saves, and playback continuity. Legacy media progress is retained but ignored; tag-group review progress remains supported.
...
## Separate backend milestone — Atomic actions

- [ ] Design atomic multi-step metadata actions as a separately scoped backend capability.
- [ ] Preserve explicit partial-failure reporting. The extension cannot provide atomic rollback.
...
Remaining public-API boundaries: visual-similarity queue evaluation is unavailable; saved-filter writes lack atomic compare-and-swap, so simultaneous cross-browser writes remain a race despite revision checks; metadata actions wait approximately 1.1 seconds before refreshing because Cove caches identical filtered queries for one second and exposes no invalidation hook. Present/absent assessments use sequential read-modify-write requests because the API has no transaction, concurrency token, or field-specific mutation endpoint; other writers can introduce later contradictions, and library-wide contradiction discovery and resolution remain follow-up work. Tag-bin counts cover the loaded page, and resumption uses identity within the saved page or its nearest index rather than scanning the whole library. Registry publication and its post-merge compatibility floor remain deferred. Atomic actions remain a separate backend milestone.

## Audio delivery notes

Data Quality now reviews audios as well as videos, through two additional review entity types: `audio` (single-item only) and `audioPerformerOccurrence`. The queue, workspace, tag editing, actions, assessments, batch occurrence application, and undo are shared with the video path through one media-kind abstraction: the review's entity type selects the collection (`/api/videos` or `/api/audios`), the tag-application host type, the custom-field entity type, the filter criteria and sort options, the detail route, and the player. Audio reviews never render the card grid, so the multiple-item layout, card annotations, card tag bins, the grid/wall preference, and select-all-on-load stay video-only, and the model rejects a saved audio review that carries them.

Because an audio shows nothing while it plays, the description renders under the player in both audio review kinds through Cove's narrative-text renderer, with a collapse control whose state is per-browser and deliberately outside the saved review.

Three Cove changes were required and made alongside this work: `@cove/runtime/components` now exports the audio player, audio card, audio filter criteria, audio sort options and narrative text; the audio performer filter honors `performerOccurrenceTagsCriterion` (and exact performer ids) on the same performer link, as the video filter already did; and audio bulk updates accept merge-mode custom fields, which the occurrence absence field needs. `minCoveVersion` is therefore raised and must be confirmed against the actual merged Cove dev version before publication.
