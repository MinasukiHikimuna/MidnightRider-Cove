    ...
    <fieldset>
      ...
      <label>
        ...
        >
          {!VIDEO_SORT_OPTIONS.some((o) => o.value === filter.sort) &&
            filter.sort != null && (
              <option value={String(filter.sort)}>{String(filter.sort)}</option>
            )}
      ...
      <label>
        ...
        <input
          ...
          onChange={(e) =>
            updateFilter({
              perPage: Math.max(1, Math.min(100, Number(e.target.value) || 40)),
            })
          }
        />
      </label>
      ...
      <p>
        {Object.keys(draft.view.objectFilter).length
          ...
          : "No video filters"}
        . Existing search and filter settings are preserved until you change
        them.
      </p>
      {filtersOpen && (
