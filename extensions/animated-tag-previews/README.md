...
- Cove 1.1.0 or newer, containing the entity-media, player-tool, minimal-API authorization, and
  MIME-preserving blob contracts used by this extension.
...
`<Cove data>/extensions/com.midnightrider.animated-tag-previews` and keep `extension.json` at that package root. Do not
...
Run from the extension package directory:

```bash
cd extensions/animated-tag-previews
cd src/AnimatedTagPreviews/ui
npm ci
npm test
npm run build
cd ../../..
dotnet restore AnimatedTagPreviews.slnx --property:UseLocalCoveSource=false --property:UseLocalCoveCore=false
dotnet test AnimatedTagPreviews.slnx --configuration Release --no-restore --property:UseLocalCoveSource=false --property:UseLocalCoveCore=false
```

The repository release workflow performs the same frontend and backend validation, publishes the
extension, validates its package contents, and creates
`com.midnightrider.animated-tag-previews-<version>.zip` plus its SHA-256 checksum. Build output is
disposable; user-generated previews are never inputs to or outputs from this packaging process.
