...
describe("preview editor", () => {
  ...
  it("pauses the host and replaces the crop editor with the generated candidate review", async () => {
    ...
    expect(candidate).toHaveAttribute("src", "/api/extensions/com.midnightrider.animated-tag-previews/videos/12/tags/77/candidates/candidate-1/media");
  ...
  it("discards the candidate and restores the exact editor state and loop on Reset", async () => {
    ...
    expect(requests).toContainEqual(expect.objectContaining({
      path: "/extensions/com.midnightrider.animated-tag-previews/videos/12/tags/77/candidates/candidate-1",
  ...
  it("publishes only on Approve, refreshes the index, and closes the editor", async () => {
    ...
    expect(requests).toContainEqual(expect.objectContaining({
      path: "/extensions/com.midnightrider.animated-tag-previews/videos/12/tags/77/candidates/candidate-1/approve",
...
describe("animated tag cover editor", () => {
  ...
  it("uploads a replacement image before deleting the animated preview", async () => {
    ...
    await waitFor(() => expect(requests).toContainEqual(expect.objectContaining({
      path: "/extensions/com.midnightrider.animated-tag-previews/tags/7/media",
      init: expect.objectContaining({ method: "DELETE" }),
    })));
