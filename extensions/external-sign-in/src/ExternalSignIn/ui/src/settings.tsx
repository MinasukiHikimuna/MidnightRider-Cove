...
import {
  externalSignInApi,
  DEFAULT_SETTINGS,
  type ExternalSignInSettings as Settings,
  type ExternalSignInSettingsUpdate,
...
export function ExternalSignInSettings() {
  ...
  const [status, setStatus] = useState("Loading external sign-in settings…");
  ...
  useEffect(() => {
    ...
    externalSignInApi.getSettings()
      ...
      .catch(reason => {
        if (active) setStatus(reason instanceof Error ? reason.message : "Could not load external sign-in settings.");
      })
  ...
  const payload = (): ExternalSignInSettingsUpdate => ({
  ...
  const save = async () => {
    setBusy(true);
    setStatus("Saving external sign-in settings…");
    try {
      const saved = await externalSignInApi.saveSettings(payload());
      applySettings(saved);
      setClientSecrets({});
      setClearedSecrets(new Set());
      setStatus("Settings saved. Existing identities remain linked by immutable provider authority and subject.");
    } catch (reason) {
      setStatus(reason instanceof Error ? reason.message : "Could not save external sign-in settings.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const testOidc = async (provider: DraftProvider) => {
    ...
    try {
      await externalSignInApi.testOidc(provider.id);
  ...
  return (
    <section className="external-sign-in-settings" aria-labelledby="external-sign-in-title">
      <header>
        <h3 id="external-sign-in-title">External sign-in</h3>
        <p>
      ...
      <fieldset disabled={busy}>
        ...
        <div className="external-sign-in-grid">
          <label>
            Cove public URL
            <input
              type="url"
        ...
        <label className="external-sign-in-check external-sign-in-warning">
          <input
            type="checkbox"
        ...
        <p className="external-sign-in-help">
          Callback for every provider: <code>/api/plugins/com.midnightrider.external-sign-in/oidc/callback</code>
        </p>
      </fieldset>

      <div className="external-sign-in-provider-heading">
      ...
      {providers.length === 0 ? <p className="external-sign-in-help">No OIDC providers are configured.</p> : null}
      {providers.map((provider, index) => (
        <fieldset key={provider.draftKey} disabled={busy}>
          <legend>{provider.buttonLabel || `OIDC provider ${index + 1}`}</legend>
          <label className="external-sign-in-check">
            <input
              type="checkbox"
          ...
          <div className="external-sign-in-grid">
            ...
            <label>
              Issuer {provider.id ? <span className="external-sign-in-muted">(immutable)</span> : null}
              <input
                type="url"
            ...
            <label className="external-sign-in-wide">
              Scopes
          ...
          <div className="external-sign-in-actions">
            <button type="button" onClick={() => void testOidc(provider)} disabled={!provider.id || !provider.clientSecretConfigured}>
              Test saved provider
            </button>
            <button type="button" className="external-sign-in-danger" onClick={() => removeProvider(provider)}>
              Delete provider
            </button>
          </div>
        </fieldset>
      ...
      <fieldset disabled={busy}>
        ...
        <div className="external-sign-in-grid">
          ...
          <label>
            Authority ID {settings.trustedHeaderProviderId ? <span className="external-sign-in-muted">(disable and unlink to replace)</span> : null}
            <input
        ...
        <p className="external-sign-in-help external-sign-in-warning">
      ...
      <p className="external-sign-in-status" role="status" aria-live="polite">{status}</p>
    </section>
  );
}
