...
public sealed class ExternalSignInExtensionTests
{
    [Fact]
    public async Task Manifest_advertises_each_ready_provider_with_login_and_link_actions()
    {
        ...
        Assert.Contains(extension.GetUIManifest().SettingsPanels,
            panel => panel.ComponentName == "ExternalSignInSettings");

        var update = await provider.GetRequiredService<IExternalSignInSettingsStore>()
            .UpdateAsync(ExternalSignInSettingsTests.ValidUpdate() with
            {
                OidcProviders =
                [
                    ExternalSignInSettingsTests.ValidProviderUpdate(),
                    ExternalSignInSettingsTests.ValidProviderUpdate() with
                    {
                        Id = "second",
        ...
        Assert.Collection(
            ...
            method =>
            {
                ...
                Assert.Equal($"{ExternalSignInExtension.ApiBase}/oidc/authentik/start", method.StartUrl);
                Assert.Equal($"{ExternalSignInExtension.ApiBase}/oidc/authentik/link/start", method.LinkStartUrl);
            },
    ...
    public async Task Trusted_header_manifest_is_linkable_but_hidden_from_the_login_page()
    {
        ...
        await using var provider = services.BuildServiceProvider();
        await extension.InitializeAsync(provider);
        var update = ExternalSignInSettingsTests.ValidUpdate() with
        {
            OidcProviders = [],
            CovePublicUrl = "",
        ...
        Assert.True((await provider.GetRequiredService<IExternalSignInSettingsStore>()
        ...
        Assert.Equal($"{ExternalSignInExtension.ApiBase}/trusted-header/link/start", method.LinkStartUrl);
    }
    ...
    public async Task Start_and_callback_complete_a_browser_bound_subject_flow_and_preserve_return_url()
    {
        ...
        var start = await client.GetAsync($"{ExternalSignInExtension.ApiBase}/oidc/authentik/start?returnUrl=%2Fsettings%3Ftab%3Dsecurity");
        var callback = await client.GetAsync(
            $"{ExternalSignInExtension.ApiBase}/oidc/callback?state={Uri.EscapeDataString(protocol.Flow!.State)}&code=provider-code");
    ...
    public async Task Unlinked_subject_returns_a_specific_non_sensitive_login_error()
    {
        ...
        await using var app = await StartAppAsync(protocol, sessions);
        var client = app.GetTestClient();
        await client.GetAsync($"{ExternalSignInExtension.ApiBase}/oidc/authentik/start");
    ...
    public async Task Oidc_link_flow_prepares_confirmation_instead_of_logging_in()
    {
        ...
        var start = await client.PostAsync(
            $"{ExternalSignInExtension.ApiBase}/oidc/authentik/link/start",
            content: null);
        var callback = await client.GetAsync(
            $"{ExternalSignInExtension.ApiBase}/oidc/callback?state={Uri.EscapeDataString(protocol.Flow!.State)}&code=provider-code");
    ...
    {
        var links = new FakeLinks();
        var settings = ExternalSignInSettingsTests.ValidSettings() with
        {
            TrustedHeaderEnabled = true,
            TrustedHeaderProviderId = "proxy-authority",
        ...
        var request = new HttpRequestMessage(
            HttpMethod.Post,
            $"{ExternalSignInExtension.ApiBase}/trusted-header/link/start");
    ...
    public async Task Wrong_browser_is_rejected_before_exchange_without_consuming_the_flow()
    {
        ...
        await using var app = await StartAppAsync(protocol, sessions);
        var client = app.GetTestClient();
        await client.GetAsync($"{ExternalSignInExtension.ApiBase}/oidc/authentik/start?returnUrl=%2Fsettings");
        var callback = $"{ExternalSignInExtension.ApiBase}/oidc/callback?state={Uri.EscapeDataString(protocol.Flow!.State)}&code=provider-code";
    ...
    public async Task Settings_response_exposes_secret_presence_per_provider_only()
    {
        ...
        var response = await app.GetTestClient()
            .GetFromJsonAsync<ExternalSignInSettingsResponse>($"{ExternalSignInExtension.ApiBase}/settings");
    ...
    public async Task Provider_with_existing_links_cannot_be_deleted_but_can_be_disabled()
    {
        ...
        var remove = await client.PutAsJsonAsync(
            $"{ExternalSignInExtension.ApiBase}/settings",
            ExternalSignInSettingsTests.ValidUpdate() with { OidcProviders = [] });
        var disable = await client.PutAsJsonAsync(
            $"{ExternalSignInExtension.ApiBase}/settings",
            ExternalSignInSettingsTests.ValidUpdate() with
            {
                OidcProviders =
                [
                    ExternalSignInSettingsTests.ValidProviderUpdate() with
    ...
    {
        var identities = new FakeIdentities { ProviderLinkCount = 1 };
        var current = ExternalSignInSettingsTests.ValidSettings() with
        {
        ...
        var request = ExternalSignInSettingsTests.ValidUpdate() with
        {
    ...
    public void Routes_explicitly_declare_anonymous_authenticated_and_admin_access()
    {
        ...
        var settings = routes.Single(endpoint => endpoint.DisplayName?.StartsWith($"HTTP: GET {ExternalSignInExtension.ApiBase}/settings", StringComparison.Ordinal) == true);
    ...
    private static async Task<WebApplication> StartAppAsync(
        ...
        ExternalSignInSettings? configuredSettings = null)
    {
    ...
    private static void RegisterServices(
        ...
        ExternalSignInSettings? configuredSettings = null)
    {
        var settings = configuredSettings ?? ExternalSignInSettingsTests.ValidSettings();
        var fixedSettings = new FixedSettingsStore(settings);
        services.AddSingleton<IExternalSignInSettingsProvider>(fixedSettings);
        services.AddSingleton<IExternalSignInSettingsStore>(fixedSettings);
    ...
    private static ExternalSignInExtension Extension()
    {
        var extension = new ExternalSignInExtension();
        ((IManifestAware)extension).ApplyManifest(new ExtensionManifestFile
        {
            Id = ExternalSignInExtension.ExtensionId,
    ...
    private sealed class FixedSettingsStore(ExternalSignInSettings settings)
        : IExternalSignInSettingsStore
    {
        public ExternalSignInSettings Current { get; private set; } = settings;
        public Task LoadAsync(CancellationToken ct = default) => Task.CompletedTask;
        public Task<ExternalSignInSettingsValidation> UpdateAsync(
