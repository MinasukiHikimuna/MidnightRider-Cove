# Host-side Authentik setup for External Sign-In development
...
5. Add exactly this **Strict** redirect URI:
   ...
   ```text
   <COVE_PUBLIC_URL>/api/plugins/com.midnightrider.external-sign-in/oidc/callback
   ```
...
Install and enable `com.midnightrider.external-sign-in`. In **Settings > External sign-in**:
...
```text
/api/plugins/com.midnightrider.external-sign-in/oidc/<provider-id>/start
/api/plugins/com.midnightrider.external-sign-in/oidc/<provider-id>/link/start
/api/plugins/com.midnightrider.external-sign-in/oidc/callback
```
