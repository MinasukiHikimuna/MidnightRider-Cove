...
public sealed class AuthMiddlewareSettingsTests
{
    ...
    {
        var generated = AuthMiddlewareSettingsValidator.ValidateUpdate(
            ValidUpdate() with { OidcProviders = [ValidProviderUpdate() with { Id = null }] },
            AuthMiddlewareSettings.Default);
        var duplicate = AuthMiddlewareSettingsValidator.ValidateUpdate(
            ValidUpdate() with
            {
                OidcProviders =
                [
    ...
    public void Persisted_oidc_provider_issuer_is_immutable()
    {
        ...
        var result = AuthMiddlewareSettingsValidator.ValidateUpdate(
            ValidUpdate() with
            {
                OidcProviders =
                [
    ...
    {
        var response = AuthMiddlewareSettingsResponse.From(ValidSettings());
    ...
    public async Task Legacy_single_provider_settings_migrate_without_treating_username_as_a_stable_header_subject()
    {
        ...
        var store = new AuthMiddlewareSettingsStore(() => storage);
    ...
    internal static AuthMiddlewareSettings ValidSettings() => AuthMiddlewareSettings.Default with
    {
        CovePublicUrl = "https://cove.example.invalid",
        OidcProviders =
        [
    ...
    internal static AuthMiddlewareSettingsUpdate ValidUpdate() => new()
    {
        CovePublicUrl = "https://cove.example.invalid",
