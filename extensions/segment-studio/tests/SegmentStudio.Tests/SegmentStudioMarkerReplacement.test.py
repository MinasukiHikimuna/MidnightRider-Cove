...
class FakeTarget:
    ...
    def verify(self, plan=None):
        ...
        return {
            ...
            "shotBoundaryCount": self.target.get(
                "shotBoundaryCount", len(self.target.get("shotBoundaries", []))
            ),
            "shotBoundaryFingerprint": MIGRATION.shot_boundary_state_fingerprint(
                self.target.get("shotBoundaries", [])
            ),
...
class ReplacementApplyTests(unittest.TestCase):
    def test_apply_locks_replans_verifies_and_commits(self):
        ...
        self.assertEqual(result["appliedMode"], "segments-only")
        self.assertFalse(result["shotBoundariesApplied"])
        self.assertEqual(result["deferredShotBoundaryCount"], 1)
        self.assertEqual(result["postCheck"]["shotBoundaryCount"], 0)
        self.assertNotEqual(
            result["postCheck"]["shotBoundaryFingerprint"],
            plan.result["plannedShotBoundaryFingerprint"],
        )
        self.assertEqual(
            adapter.events,
