...
test("a held tag choice is replaced, kept or cleared against the server tag", () => {
  const segment = { id: -5, tagId: 1, tagName: "Original" };
...
test("a held tag waits only while the tag field is open on its segment alone", () => {
  const editing = { tagEditing: true, selectedSegmentIds: [42], activeSegmentId: 42 };
  assert.equal(ui.heldTagReady(editing, 42), false);
  assert.equal(ui.heldTagReady({ ...editing, tagEditing: false }, 42), true);
  assert.equal(ui.heldTagReady({ ...editing, selectedSegmentIds: [7], activeSegmentId: 7 }, 42), true);
  assert.equal(ui.heldTagReady({ ...editing, selectedSegmentIds: [42, 7] }, 42), true);

  const held = { id: 42, itemId: null, nativeSegmentId: 42 };
  let list = ui.addPendingChange([], { op: "patch", targets: [held], values: { tagId: 9 }, meta: { kind: "held-tag", tagName: "Held" } });
  list = ui.addPendingChange(list, { op: "patch", targets: [{ id: 7, itemId: null, nativeSegmentId: 7 }], values: { startSec: 3 } });
  assert.equal(ui.heldTagChangeFor(list, held).meta.tagName, "Held");
  assert.equal(ui.heldTagChangeFor(list, { id: 7, itemId: null, nativeSegmentId: 7 }), null);
  assert.equal(ui.heldTagChangeFor(ui.settlePendingChange(list, list[0].id), held), null);
  assert.deepEqual(ui.pendingInsertedSegments(ui.addPendingChange(list, { op: "insert", segment: { id: -1 } })), [{ id: -1 }]);
});
